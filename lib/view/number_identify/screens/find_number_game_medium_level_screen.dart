...
class FindNumberGameMediumLevelScreen extends StatefulWidget {
  const FindNumberGameMediumLevelScreen({super.key});

  @override
  State<FindNumberGameMediumLevelScreen> createState() =>
      _FindNumberGameMediumLevelScreenState();
}
        ...
        Row(
          children: [
            GestureDetector(
                behavior: HitTestBehavior.opaque,
                onTap: () async {
                  highScore = await StorageService().getMediumNumberScore();
                  mediumNumberTimer =
                      await StorageService().getMediumNumberTimer();
                  dev.log('High Score $highScore');
                  dev.log('Expected Score $expectedNumber');
