...
class NumberGuessingGameHardLevelScreen extends StatefulWidget {
  const NumberGuessingGameHardLevelScreen({super.key});

  @override
  State<NumberGuessingGameHardLevelScreen> createState() =>
      _NumberGuessingGameHardLevelScreenState();
}

class _NumberGuessingGameHardLevelScreenState
    extends State<NumberGuessingGameHardLevelScreen> {
  int currentRound = 1;
  int numberOfBoxes = 2;
  List<int> boxNumbers = [];
  List<bool> revealed = [];
  ...
  getHighScoreForEasy() async {
    getHighScoreValue = await StorageService().getNumberGuessHard();
          ...
          children: [
            ...
            Consumer<NumberScreenProvider>(
              builder: (context, best, child) {
                return Text('Best : ${best.getHardHighScore}',
                    style: GoogleFonts.gamjaFlower(
                        fontSize: 30.sp,
                        fontWeight: FontWeight.w700,
                        fontStyle: FontStyle.normal,
  ...
  void generateBoxes() {
    ...
    List<int> remainingNumbers = List.generate(21, (index) => index + 1)
    ...
    setState(() {
      ...
      if (boxNumbers[tappedIndex] == currentRound) {
        ...
        Timer(const Duration(seconds: 1), () async {
          ...
          highScore = await StorageService().getNumberGuessHard();
          if (highScore == 0 || currentRound > highScore) {
            StorageService().setNumberGuessHard(currentRound - 1);
            Provider.of<NumberScreenProvider>(context, listen: false)
                .addHardHighScore(currentRound - 1);
          }
        });
      } else {
        backGroundColor = List.generate(
