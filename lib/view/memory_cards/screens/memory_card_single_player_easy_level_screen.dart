...
class MemoryCardSinglePlayerEasyLevelScreen extends StatefulWidget {
  const MemoryCardSinglePlayerEasyLevelScreen({super.key});

  @override
  State<MemoryCardSinglePlayerEasyLevelScreen> createState() =>
      _MemoryCardSinglePlayerEasyLevelScreenState();
}

class _MemoryCardSinglePlayerEasyLevelScreenState
    extends State<MemoryCardSinglePlayerEasyLevelScreen>
        ...
        if (shuffledIcons[firstIndex] == shuffledIcons[secondIndex]) {
          Future.delayed(const Duration(milliseconds: 300), () {
            setState(() async {
              ...
              highScore = await StorageService().getSoloEasyNumberScore();
              hardNumberTimer = await StorageService().getSoloEasyNumberTimer();
              dev.log('High Score $highScore');
              dev.log('Matched Cards $matchedCards');
              dev.log('Timer $timerRun');
              if (highScore == 0 || matchedCards > highScore) {
                StorageService().setSoloEasyNumberScore(matchedCards);
                highScore = await StorageService().getSoloEasyNumberScore();
                StorageService().setSoloEasyNumberTimer(timerRun);
                hardNumberTimer =
                    await StorageService().getSoloEasyNumberTimer();
