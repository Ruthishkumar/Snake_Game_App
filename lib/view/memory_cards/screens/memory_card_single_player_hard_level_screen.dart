...
class MemoryCardSinglePlayerHardLevelScreen extends StatefulWidget {
  const MemoryCardSinglePlayerHardLevelScreen({super.key});

  @override
  State<MemoryCardSinglePlayerHardLevelScreen> createState() =>
      _MemoryCardSinglePlayerHardLevelScreenState();
}

class _MemoryCardSinglePlayerHardLevelScreenState
    extends State<MemoryCardSinglePlayerHardLevelScreen>
        ...
        if (shuffledIcons[firstIndex] == shuffledIcons[secondIndex]) {
          Future.delayed(const Duration(milliseconds: 300), () {
            setState(() async {
              ...
              highScore = await StorageService().getSoloHardNumberScore();
              hardNumberTimer = await StorageService().getSoloHardNumberTimer();
              dev.log('High Score $highScore');
              dev.log('Matched Cards $matchedCards');
              dev.log('Timer $timerRun');
              if (highScore == 0 || matchedCards > highScore) {
                StorageService().setSoloHardNumberScore(matchedCards);
                highScore = await StorageService().getSoloHardNumberScore();
                StorageService().setSoloHardNumberTimer(timerRun);
                hardNumberTimer =
                    await StorageService().getSoloHardNumberTimer();
